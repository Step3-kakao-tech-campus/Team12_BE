...
public class UserController {
    ...
    public ResponseEntity<ApiUtils.ApiResult<GetUserRp>> userAuthStatus(@Login Long userId) {
        GetUserRp responseDTO = userService.userAuthStatus(userId);
        return ResponseEntity.ok(ApiUtils.success(responseDTO));
    }
    ...
    public ResponseEntity<ApiUtils.ApiResult<UploadImageRp>> uploadImage(@ModelAttribute @Valid UpdateUserImageRq requestDTO,
                                                                  @Login Long userId) {
        UploadImageRp responseDTO = userService.uploadImage(requestDTO.image(), userId);
        return ResponseEntity.ok(ApiUtils.success(responseDTO));
    }
    ...
    public ResponseEntity<ApiUtils.ApiResult<CustomPage<ReadWriterBoardListRp>>> myPageRequesterList(
            ...
            @RequestParam(value = "limit",defaultValue = "10") int size){
        Slice<ReadWriterBoardListRp> responseDTO = boardService.myPageRequesterList(userId, lastBoardId, size);
    ...
    public ResponseEntity<ApiUtils.ApiResult<ReadWriterBoard>> myPageRequesterDetail(@PathVariable("boardId") Long boardId){
        ReadWriterBoard responseDTO = boardService.myPageRequesterDetail(boardId);
        return ResponseEntity.ok(ApiUtils.success(responseDTO));
    }
    ...
    public ResponseEntity<ApiUtils.ApiResult<CustomPage<ReadPickerBoardListRp>>> myPagePickerList(@RequestParam(value = "offset",required = false) Long lastBoardId,
                                                                                                  @RequestParam(value = "limit",defaultValue = "10") int size,
                                                                                                  @Login Long userId) {
        Slice<ReadPickerBoardListRp> responseDTO = boardService.myPagePickerList(lastBoardId,size,userId);
        return ResponseEntity.ok(ApiUtils.success(new CustomPage<>(responseDTO)));
    }
    ...
    public ResponseEntity<ApiUtils.ApiResult<ReadPickerBoardRp>> myPagePickerDetail(@PathVariable("boardId") Long boardId,
                                                                                    @Login Long userId) {
        return ResponseEntity.ok(ApiUtils.success(boardService.pickerBoardDetail(boardId, userId)));
    }
}